...
function getUserBooks(req, res) {
  ...
  const query = `SELECT Book.*, AVG(rating) AS rating
  FROM Book LEFT JOIN ReadingList ON Book.isbn = ReadingList.isbn
  LEFT JOIN Ratings ON Book.isbn = Ratings.isbn
  WHERE ReadingList.user_id = ${userid}
  GROUP BY Book.ISBN, TITLE, AUTHOR, GENRE, LANGUAGE, COVER, PUBLISHER, YEAR_PUBLISHED, PRICE, NUM_PAGES`;
  runQuery(query, (result) => {
    res.json(result.rows);
  });
}
...
function getUserRated(req, res) {
  const userid = req.body.userid;
  const query = `SELECT Book.*, AVG(r2.rating) AS rating
  FROM Book LEFT JOIN Ratings r1 ON Book.isbn = r1.isbn
  LEFT JOIN Ratings r2 ON Book.isbn = r2.isbn
